...
public interface OfertaRepository extends JpaRepository<Oferta, Long>, OfertaRepositoryCustom{
	...
	@Query(value = "SELECT * FROM oferta WHERE estat = :estat WHERE data_de_publicacio > DATE_SUB(now(), INTERVAL 3 MONTH) ORDER BY data_de_publicacio DESC", nativeQuery = true)
